...
def _read_if_needed(anndata_mudata_path_or_obj):
    ...
    raise AssertionError(
        "Expected 'Path', 'str' to MuData/AnnData " "file or MuData/AnnData object."
    )


def _assert_same_annotation_object_class(left, right):
    assert type(left) is type(right), (
        f"Two objects are not of the same class:"
        f"\n[Left]:{type(left)}\n[right]:{type(right)}"
    )
...
def assert_shape_equal(
    ...
    if left.shape != right.shape:
        raise AssertionError(
            f"{type(left).__name__} shapes differ:"
            f"\n[left]:{left.shape}\n[right]:{right.shape}"
        )
...
def assert_obs_names_equal(
    left: AnnotationObjectOrPathLike, right: AnnotationObjectOrPathLike, *args, **kwargs
):
    left, right = _read_if_needed(left), _read_if_needed(right)
    _assert_same_annotation_object_class(left, right)
...
def _assert_frame_equal(
    left, right, sort=False, promote_precicion=False, *args, **kwargs
):
    if sort:
        left, right = left.sort_index(inplace=False), right.sort_index(inplace=False)
        left, right = (
            left.sort_index(axis=1, inplace=False),
            right.sort_index(axis=1, inplace=False),
        )
...
def assert_annotation_frame_equal(
    annotation_attr: Literal["obs", "var"],
    left: AnnotationObjectOrPathLike,
    ...
    **kwargs,
    ...
    if annotation_attr not in ("obs", "var"):
...
def _assert_layer_equal(left, right):
    if issparse(left):
        ...
        if left.getformat() != right.getformat():
            raise AssertionError(
                "Layers format differ:"
                f"\n[left]:{left.getformat()}\n[right]: {right.getformat()}"
            )
        assert np.all(
            left.indices == right.indices
        ), "Layers differ: indices are not the same"
        assert np.all(
            left.indptr == right.indptr
        ), "Layers differ: index pointers are not the same"
        ...
        if issparse(right):
            raise AssertionError("Layers differ:\n[left]: not sparse\n[right]: sparse")
        np.testing.assert_allclose(
            left, right, rtol=1e-5, err_msg="Layers data differs.", equal_nan=True
        )


def assert_layers_equal(
    left: AnnotationObjectOrPathLike, right: AnnotationObjectOrPathLike
):
    left, right = _read_if_needed(left), _read_if_needed(right)
    _assert_same_annotation_object_class(left, right)
...
def assert_multidimensional_annotation_equal(
    annotation_attr: Literal["obsm", "varm"], left, right, sort=False
):
    if annotation_attr not in ("obsm", "varm"):
    ...
    ), f"Keys of {annotation_attr} differ:\n[left]:{left_keys}\n[right]:{right_keys}"
    for left_key, left_value in left_dict.items():
        try:
            _assert_multidimensional_value_equal(
                left_value, right_dict[left_key], sort=sort
            )
        except AssertionError as e:
    ...
    if isinstance(left, MuData):
        ...
        for mod_name, modality in left.mod.items():
            try:
                assert_multidimensional_annotation_equal(
                    annotation_attr, modality, right[mod_name], sort=sort
                )
            except AssertionError as e:
...
def assert_annotation_objects_equal(
    left: AnnotationObjectOrPathLike,
    right: AnnotationObjectOrPathLike,
    check_data=True,
    sort=True,
    ...
    assert_annotation_frame_equal(
        "obs", left, right, sort=sort, promote_precicion=promote_precision
    )
    assert_annotation_frame_equal(
        "var", left, right, sort=sort, promote_precicion=promote_precision
    )
